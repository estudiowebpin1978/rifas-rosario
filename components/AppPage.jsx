...
export default function AppPage() {
  ...
  useEffect(() => {
    ...
    if (supabase) {
      ...
      } catch (e) { console.log('Realtime no disponible'); }
    }
  }, []);
  ...
  const handleBulkReserva = async (e) => {
    ...
    for (const num of selectedNumbers) {
      await supabase.from('boletos').update({ estado: 'reservado', nombre: reservaForm.nombre, whatsapp: reservaForm.whatsapp }).eq('numero', num).eq('producto_id', productoSeleccionado.id);
    }
  ...
  const handleReserva = async (e) => {
    ...
    setLoading(true);
    const { error } = await supabase.from('boletos').update({ estado: 'reservado', nombre: reservaForm.nombre, whatsapp: reservaForm.whatsapp }).eq('numero', seleccionado).eq('producto_id', productoSeleccionado.id);
    if (!error) {
